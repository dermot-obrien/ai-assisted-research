...
- **Regenerate Dashboard**: Run `python .ai-assisted-research/tools/generate_dashboard.py` to rebuild the interactive HTML dashboard (`dashboard.html` at the path declared by `dashboard.output_html` in `research.yaml`) from the DAG, node index, experiment logs, breakthroughs, papers, and findings-edges files.
