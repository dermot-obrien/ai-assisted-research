...
2. **Locate the Hypothesis DAG**: Identify the central `hypothesis-dag.yaml` (path from `research.yaml` -> `dag_path`).
3. **Validate the Node**: Ensure the `{node_id}` exists in the DAG and is currently `in_progress`.

### Phase 2: Extract Results from AAW
1. **Extract Metrics**: Read the `actual_performance` from `change/work-items/{WI_id}/metadata.yaml` or `progress.yaml`.
...
### Phase 4: Cascade Check (v1.2.0)
If the node status changed to `validated` or `ineffective`, **trigger cascade analysis**:
1. Invoke `/cascade {node_id}` to identify affected nodes.
2. Record cascade effects in `breakthroughs.yaml` if applicable.
3. Update the findings graph with new relationship edges.

### Phase 5: Finalize & Housekeep
1. **Commit Changes**: Commit the updated DAG and any synthesized articles.
2. **Refresh Node Index**: Regenerate `node-index.yaml` to reflect the new DAG state. A node completing may unblock its children, moving them from `blocked` to `ready`.
3. **Update Visualization**: Run `/housekeep` to refresh dashboards.
